...
public class HiveSqlScriptGenerator {
  ...
  public String generateSaveAsTable(JdbcDataConnection jdbcDataConnection, SavingTable savingTable) {
    ...
    // 1. Create Database
    final String personalDatabaseName = String.format("%s_%s", ((HiveConnection)jdbcDataConnection).getPersonalDatabasePrefix(),
        HiveNamingRule.replaceNotAllowedCharacters(savingTable.getLoginUserId()));
