...
/***/ (function(module, exports, __webpack_require__) {
	...
	var common = __webpack_require__(329);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function zs() {
	    ...
	    common.format = function (value, digits) {
	        ...
	        var addComma = function addComma(num, scope) {
	            do {
	        ...
	        var numArray = String(value).split('.');
	        numArray[0] += '.';
	    ...
	    common.numberFormat = function (value, format) {
	        ...
	        if (format.type !== 'exponent10' && format.useThousandsSep) {
	            ...
	            var arrSplitFloatPoint = String(value).split('.');
	        ...
	        if (value && format.type != 'exponent10' && format.decimal > 0) {
	            ...
	            if (stringValue.indexOf('.') == -1) {
	                value += '.';
	                for (var num = 0; num < format.decimal; num++) {
	                    value += '0';
	                }
	            } else {
	                for (var _num = stringValue.split('.')[1].length; _num < format.decimal; _num++) {
	                    value += '0';
	                }
	            }
	        }
	        ...
	        if (customSymbol && customSymbol.value.trim().length > 0) {
	            ...
	            var symbolValue = customSymbol.value.trim();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function style(zs) {
		var ui = zs.ui;
		var PivotStyle = ui.style;
		var common = zs.ui.common;
		...
		PivotStyle.setStyleVerticalCells = function ($elm, $bodyRow, cellStyle) {
			...
			var parentVal = $elm.attr('data-parent-vals');
			var elmVals = $elm.attr('title');
			var strHierachy = parentVal ? parentVal + '||' + elmVals : elmVals;
	  ...
	  */
		...
		PivotStyle.setStyleHorizontalCells = function ($elm, $bodyRow, cellStyle) {
			var cellHeight = this._settings.cellHeight;

			// Body Cell Index 범위 초기값 정의
			var startIdx = $elm.parent().prevAll().length;
			var endIdx = startIdx + $elm.css('height').replace(common.__regexpText, '') / cellHeight;

			// xAxis 하위 cell 스타일 적용
			var heightCnt = $elm.css('height').replace(common.__regexpText, '') / cellHeight;
			$elm.nextAll().each(function (idx, axisElm) {
				var $axisElm = $(axisElm);
				var currTargetKey = $axisElm.attr('data-key');
				var targetCnt = heightCnt;
				while (0 < targetCnt && 0 < $axisElm.length) {
					$axisElm.addClass(cellStyle);
					var currHeightCnt = $axisElm.css('height').replace(common.__regexpText, '') / cellHeight;
					targetCnt -= currHeightCnt;
					$axisElm = $axisElm.parent();
					for (var idx2 = 0; idx2 < currHeightCnt; idx2++) {
		...
		PivotStyle.setClickStyle = function () {
			...
			// 초기 데이터 정의
			var selectedBodyStyle = PivotStyle.cssClass.bodySelected;
			var selectedAxisStyle = PivotStyle.cssClass.axisSelected;
			var selectAxisBodyStyle = PivotStyle.cssClass.axisSelectedBody;
			var disabledAxisStyle = PivotStyle.cssClass.axisDisable;
			...
			// 공통 엘레멘트 사전 정의
			var $bodyRow = $container.find('.' + PivotStyle.cssClass.body + ' .' + PivotStyle.cssClass.bodyWrap + ' .' + PivotStyle.cssClass.bodyRow);
			var $xAxis = $container.find('.' + PivotStyle.cssClass.head);
			var $yAxis = $container.find('.' + PivotStyle.cssClass.bodyFrozen);
			...
			function targetFilter(index, elm, selectedData) {
				...
				if (selectedData.data === $elm.text() && selectedData.key === $elm.attr('data-key')) {
					if ($elm.is('[data-parent-keys]')) {
						var arrKeys = $elm.attr('data-parent-keys').split('||');
						var arrVals = $elm.attr('data-parent-vals').split('||');
						for (var idx = 0, nMax = arrKeys.length; idx < nMax; idx++) {
			...
			if (common.SELECT_MODE.MULTI === this._settings.axisSelectMode) {
				var _loop = function _loop(idx, nMax) {
					var selectedData = objViewer._axisDataset[idx];
					var $target = $xAxis.find('.' + PivotStyle.cssClass.headCell).filter(function (idx, elm) {
				...
				for (var idx = 0, nMax = objViewer._axisDataset.length; idx < nMax; idx++) {
					_loop(idx, nMax);
				} // for - objViewer._axisDataset

				var _loop2 = function _loop2(idx, nMax) {
					var selectedData = objViewer._axisDataset[idx];
					var $target = $yAxis.find('.' + PivotStyle.cssClass.bodyCell).filter(function (idx, elm) {
						return targetFilter(idx, elm, selectedData);
					});
					if (0 < $target.length) {
						$target.attr('data-selected', 'Y');
			...
			// 싱글 선택
					...
					if ('X' === this._selectedAxis) {
						var _loop3 = function _loop3(idx, nMax) {
							var selectedData = objViewer._axisDataset[idx];
							var $target = $xAxis.find('.' + PivotStyle.cssClass.headCell).filter(function (idx, elm) {
								return targetFilter(idx, elm, selectedData);
							});
					...
					// Y Axis
							...
							var _loop4 = function _loop4(idx, nMax) {
								var selectedData = objViewer._axisDataset[idx];
								var $target = $yAxis.find('.' + PivotStyle.cssClass.bodyCell).filter(function (idx, elm) {
									return targetFilter(idx, elm, selectedData);
								});
			...
			var zProp = this._settings.zProperties;
			var $bodyContainer = $container.find('.' + PivotStyle.cssClass.body + ' .' + PivotStyle.cssClass.bodyWrap);
			for (var idx = 0, nMax = this._items.length; idx < nMax; idx++) {
				var objItemInfo = this._items[idx];
				for (var zIdx = 0, zMax = zProp.length; zIdx < zMax; zIdx++) {
					var strKey = zProp[zIdx].name;
					if (objItemInfo.selectInfo && objItemInfo.selectInfo[strKey]) {
						var $elmCell = $bodyContainer.find('.' + PivotStyle.cssClass.bodyCell + '[data-idx=' + idx + '][data-key="' + strKey + '"]');
						$elmCell.addClass(selectedBodyStyle);
					}
				}
			} // end for - _items

			var dataDirectionToVertical = common.DATA_COL_MODE.LEFT === this._settings.dataColumnMode ? 1 : 0;
			var dataDirectionToHorizontal = common.DATA_COL_MODE.TOP === this._settings.dataColumnMode ? 1 : 0;
			var xMax = dataDirectionToVertical * this._xItems.length + dataDirectionToHorizontal * this._xItems.length * this._settings.zProperties.length;
			var yMax = dataDirectionToHorizontal * this._yItems.length + dataDirectionToVertical * this._yItems.length * this._settings.zProperties.length;
			for (var colIdx = 0; colIdx < xMax; colIdx++) {
				for (var rowIdx = 0; rowIdx < yMax; rowIdx++) {
		...
		PivotStyle.getSelectedAxisData = function ($elm, eventAxis, $eventAxisElm, $otherAxisElm) {
			...
			var evtData = null;
			var elmKey = $elm.attr('data-key');
			var elmData = $elm.attr('title');
			var isSelect = false;
			...
			if ('Y' === $elm.attr('data-selected')) {
				...
				// 선택된 데이터 삭제
				var selectedIdx = -1;
				for (var _idx = 0, _nMax = this._axisDataset.length; _idx < _nMax; _idx++) {
					var objData = this._axisDataset[_idx];
				...
				if (-1 < selectedIdx) {
					...
					// 선택된 데이터 추가
					var existData = false;
					for (var _idx2 = 0, _nMax2 = this._axisDataset.length; _idx2 < _nMax2; _idx2++) {
						var _objData = this._axisDataset[_idx2];
						// 20170811 Dolkkok - 중복체크시 부모 key/value 도 확인
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function viewer(zs) {
	    ...
	    var SCROLL_WIDTH = void 0;
	    var pivot = ui.pivot || (ui.pivot = {});
	    var common = ui.common;
	    var pivotStyle = ui.style;
	    ...
	    var Viewer = function () {
	         ...
	         * 그리드 초기화 함수
	         */
	        Viewer.prototype.initialize = function (items, settings) {
	            var objViewer = this;
	            ...
	            var min = !this._settings.min ? 0 : this._settings.min;
	            var max = !this._settings.max ? 0 : this._settings.max;
	            var zProp = this._settings.zProperties;
	            var showColorStep = this._settings.body.color && this._settings.body.color.showColorStep ? this._settings.body.color.showColorStep : null;

	            if (zProp && showColorStep) {
	                var arrColors = this._settings.body.color.stepColors;
	                var arrTextColors = this._settings.body.color.stepTextColors;
	                zProp.forEach(function (prop) {
	                    ...
	                    objViewer._dataCriteria[prop.name] = {
	                        ...
	                        getStep: function getStep(data) {
	                            var currStep = Math.floor((data - this.min) / this.range);
	                            this.step <= currStep && (currStep = this.step - 1);
	                            return currStep;
	                        },
	                        getTextColor: function getTextColor(data) {
	                            if (this.textColor) {
	                         ...
	                         * @returns {string}
	                         */
	                        getUserRangeColor: function getUserRangeColor(data, rangeColors) {
	                            ...
	                            try {
	                                loop1: for (var _iterator = rangeColors[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	            ...
	            if (this._isPivot) {
	                ...
	                var zPropCount = zProp.length;

	                items.columns.forEach(function (column, columnIdx) {

	                    var colNameList = _.split(column.name, '―');

	                    if (columnIdx % zPropCount !== 0) return;

	                    //20170811 Dolkkok - 피봇데이터기반으로 변경
	                    var xProps = objViewer._settings.xProperties;
	                    ...
	                    column.value.forEach(function (value, valueIdx) {
	                        ...
	                        var xGroup = objViewer._xAxisGroup;
	                        var yGroup = objViewer._yAxisGroup;
	                        ...
	                        var rowNameList = _.split(items.rows[valueIdx], '―');

	                        for (var _idx = 0, _nMax = yProps.length; _idx < _nMax; _idx++) {
	                            var _itemKey = yProps[_idx].name;
	                            var _itemValue = rowNameList[_idx];
	                            item[_itemKey] = _itemValue;

	                            if (!context.hasOwnProperty(_itemValue)) {
	                                context[_itemValue] = {};
	                            }
	                            context = context[_itemValue];

	                            if (!yGroup.hasOwnProperty(_itemValue)) {
	                                yGroup[_itemValue] = {};
	                            }
	                        ...
	                        for (var _idx2 = 0, _nMax2 = zProps.length; _idx2 < _nMax2; _idx2++) {
	                            var _itemKey2 = zProps[_idx2].name;
	                            var _itemValue2 = items.columns[columnIdx + _idx2].value[valueIdx];
	                            item[_itemKey2] = !!_itemValue2 ? _itemValue2 : null;

	                            // 요약 정보 생성
	                            var summaryKey = 0 < arrSummaryKeys.length ? arrSummaryKeys.join('||') + '||' + _itemKey2 : _itemKey2;
	                            objViewer.summaryMap[summaryKey] || (objViewer.summaryMap[summaryKey] = []);
	                            if (_itemValue2) {
	                                objViewer.summaryMap[summaryKey].push(_itemValue2);
	                            }
	                        }
	                        ...
	                        if (showColorStep) {
	                            for (var _key in item) {
	                                var criteria = objViewer._dataCriteria[_key];
	                                if (criteria) {
	                                    var itemData = item[_key];
	            ...
	            // 20171130 taeho - 피봇 / 원본 데이터형태 모두 지원하도록 변경
	                    ...
	                    items.forEach(function (item) {
	                        ...
	                        for (var _idx3 = 0, _nMax3 = yProps.length; _idx3 < _nMax3; _idx3++) {
	                            var _itemKey3 = yProps[_idx3].name;
	                            var _itemValue3 = item[_itemKey3];

	                            if (!context.hasOwnProperty(_itemValue3)) {
	                                context[_itemValue3] = {};
	                            }
	                            context = context[_itemValue3];

	                            if (!yGroup.hasOwnProperty(_itemValue3)) {
	                                yGroup[_itemValue3] = {};
	                            }
	                            yGroup = yGroup[_itemValue3];
	                        } // end for - yProps
	                        if (!yGroup.item) {
	                            objViewer._yItems.push(yGroup.item = item);
	                        }
	                        ...
	                        if (showColorStep) {
	                            for (var _key2 in item) {
	                                var criteria = objViewer._dataCriteria[_key2];
	                                if (criteria) {
	                                    var itemData = item[_key2];
	                                    criteria.min = itemData < criteria.min ? itemData : criteria.min;
	                                    criteria.max = itemData > criteria.max ? itemData : criteria.max;
	                                }
	                            }
	            ...
	            if (objViewer._settings.yAxisAutoSort) {
	                ...
	                this._yItems.sort(function (currItem, nextItem) {
	                    var arrYProps = objViewer._settings.yProperties;
	                    var nOrder = 0;
	                    try {
	                        for (var idx = 0, nMax = arrYProps.length; idx < nMax; idx++) {
	                            var propertyName = arrYProps[idx].name;
	                            // let currAxisItem = currItem[propertyName];
	                            // let nextAxisItem = nextItem[propertyName];

	                            //값이 null일 경우 정렬이 정상적이지 않는 문제로 추가된 로직
	                            var currAxisItem = currItem[propertyName] ? currItem[propertyName] : currItem[propertyName] = "null";
	                            var nextAxisItem = nextItem[propertyName] ? nextItem[propertyName] : nextItem[propertyName] = "null";
	            ...
	            // 데이터 범위 정보 설정 - Start
	            if (showColorStep) {
	                for (var _key3 in this._dataCriteria) {
	                    var objCriteria = objViewer._dataCriteria[_key3];
	            ...
	            var dataDirectionToVertical = Viewer.DATA_COL_MODE.LEFT === this._settings.dataColumnMode ? 1 : 0;
	            var dataDirectionToHorizontal = Viewer.DATA_COL_MODE.TOP === this._settings.dataColumnMode ? 1 : 0;
	            var xMax = dataDirectionToVertical * this._xItems.length + dataDirectionToHorizontal * this._xItems.length * this._settings.zProperties.length;
	            var yMax = dataDirectionToHorizontal * this._yItems.length + dataDirectionToVertical * this._yItems.length * this._settings.zProperties.length;
	            for (var colIdx = 0; colIdx < xMax; colIdx++) {
	            ...
	            var xAxisSelector = '.' + pivotStyle.cssClass.axisX;
	            var yAxisSelector = '.' + pivotStyle.cssClass.axisY;
	            var resizeHandleSelector = '.' + pivotStyle.cssClass.resizeHandle;
	            var cellSelector = '.' + pivotStyle.cssClass.body + ' .' + pivotStyle.cssClass.bodyWrap + ' .' + pivotStyle.cssClass.bodyCell; // #20161227-02 Cell Click Event 추가
	            var $container = $(this._element);
	            var $xAxis = $container.find('.' + pivotStyle.cssClass.head);
	            var $yAxis = $container.find('.' + pivotStyle.cssClass.bodyFrozen);
	            var $body = $container.find('.' + pivotStyle.cssClass.body + ' .' + pivotStyle.cssClass.bodyWrap);
	            var dragPosTop = 0;
	            //add steve
	            ...
	            .on('click', xAxisSelector, function () {
	                if ('function' === typeof objViewer._settings.onAxisXClick && 0 === $xAxis.filter('[data-disabled=Y]').length) {
	                    ...
	                    var evtData = pivotStyle.getSelectedAxisData.apply(objViewer, [$(this), 'X', $xAxis, $yAxis]);
	            ...
	            .on('click', yAxisSelector, function () {
	                if ('function' === typeof objViewer._settings.onAxisYClick && 0 === $yAxis.filter('[data-disabled=Y]').length) {
	                    // 선택된 축 정보를 조회 후, 데이터 반환
	                    var evtData = pivotStyle.getSelectedAxisData.apply(objViewer, [$(this), 'Y', $yAxis, $xAxis]);

	                    var elmKey = $(this).attr('data-key');
	            ...
	            .on('click', cellSelector, function (event) {
	                ...
	                if ('function' === typeof objViewer._settings.onBodyCellClick) {
	                        ...
	                        $.each(this.attributes, function () {
	                            if (this.specified && -1 < this.name.indexOf('data-item')) {
	                                ...
	                                if (this._isPivot) {
	                                    ...
	                                    var keyValue = _.split(this.value, '―');
	                    ...
	                    var itemKey = $target.attr('data-key');
	                    if (itemKey) {
	                        var nItemIdx = $target.attr('data-idx');
	                        var nColIdx = $target.attr('data-colIdx');
	                        var nRowIdx = $target.attr('data-rowIdx');
	                        var objItem = objViewer._items[nItemIdx];
	            ...
	            .on('drag', resizeHandleSelector, function (event) {
	                ...
	                var oldWidth = event.target.parentElement.offsetWidth;
	                ...
	                var strParent = $column.attr('data-parent-vals');
	                var strVal = $column.attr('title');
	                var strLeafColName = strParent ? strParent + '||' + strVal : strVal;
	                var dragWidth = $column.css('width').replace(/px/, '') * 1;

	                if(objViewer._leafFrozenColumnWidth[strLeafColName]) {
	                	objViewer._leafFrozenColumnWidth[strLeafColName] = dragWidth;
	                }

	                var widthKeys = Object.keys(objViewer._leafColumnWidth);
	                var contentSizeWidth = widthKeys.reduce(function (acc, item) {
	                    return acc + objViewer._leafColumnWidth[item];
	                }, 0);
	                var currentGridWidth = objViewer._elementBody.style.width.replace(/px/gi, '') * 1 - objViewer._elementBodyFrozen.style.width.replace(/px/gi, '') * 1;


	                if (currentGridWidth < contentSizeWidth) {
	                    // current state is scroll!!
	                    if (!objViewer._leafColumnWidth[strLeafColName]) {
	                        var keys = widthKeys.filter(function (item) {
	                            return -1 < item.indexOf(strLeafColName + '||');
	                        });
	                        if (keys && 0 < keys.length) {
	                            var cellWidth = dragWidth / keys.length;
	                    ...
	                    var extraWidth = currentGridWidth - dragWidth;
	                    var beforeExtraWidth = widthKeys.reduce(function (acc, val) {
	                    ...
	                    widthKeys.forEach(function (key) {
	                        if (key === strLeafColName) {
	                            ...
	                            var _keys = widthKeys.filter(function (item) {
	                                return -1 < item.indexOf(key + '||');
	                            });
	                            if (_keys && 0 < _keys.length) {
	                                var _cellWidth = dragWidth / _keys.length;
	                                _keys.forEach(function (subKey) {
	                                    objViewer._leafColumnWidth[subKey] = _cellWidth;
	                                });
	                            } else {
	        ...
	        Viewer.prototype.update = function (settings) {
	            // 20171130 taeho - 피봇 / 원본 데이터형태 모두 지원하도록 변경
	            var objItem = this._isPivot ? this._pivotData : this._items;
	            var objSettings = this._settings;
	            for (var key in settings) {
	                if (settings.hasOwnProperty(key)) {
	        ...
	        Viewer.prototype.addClassFontStyle = function (classStr, font) {
	            if (font) {
	                if (font.styles) {
	                    ...
	                    try {
	                        for (var _iterator2 = font.styles[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
	        ...
	        Viewer.prototype.addClassTextAlign = function (classStr, text, defaultAlign) {
	            if (text) {
	                if (!text.hAlign || 'auto' === text.hAlign.toLowerCase()) {
	                    classStr = classStr + ' ddp-txt-' + defaultAlign.toLowerCase();
	                } else {
	                    classStr = classStr + ' ddp-txt-' + text.hAlign.toLowerCase();
	                }

	                if (text.vAlign) {
	                    classStr = classStr + ' ddp-valign-' + text.vAlign.toLowerCase();
	                }
	            }
	            return classStr;
	        ...
	        Viewer.prototype.arrange = function () {
	            ...
	            var xPropCnt = this._isPivot ? this._settings.xProperties.length : 0;
	            var yPropCnt = this._settings.yProperties.length;
	            var zPropCnt = this._settings.zProperties.length;
	            var isSetRemark = this._settings.remark && '' !== this._settings.remark.pos ? 1 : 0;
	            var isSetTopRemark = this._settings.remark && -1 < this._settings.remark.pos.toLowerCase().indexOf('top') ? 1 : 0;

	            // #20161230-01 : 값 필드 표시 방향 선택 기능 - Start
	            var dataDirectionToVertical = Viewer.DATA_COL_MODE.LEFT === this._settings.dataColumnMode ? 1 : 0;
	            var dataDirectionToHorizontal = Viewer.DATA_COL_MODE.TOP === this._settings.dataColumnMode ? 1 : 0;
	            var isShowDataKey = this._settings.body.showAxisZ ? 1 : 0;
	            // #20161230-01 : 값 필드 표시 방향 선택 기능 - End
	            var remarkHeight = isSetRemark * 30;
	            var frozenCellWidth = this._settings.leftAxisWidth ? this._settings.leftAxisWidth : this._settings.cellWidth;

	            var availableSizeHead = {
	            ...
	            var prevScrollVertical = false;
	            var prevScrollHorizontal = false;
	            while (true) {
	                ...
	                var b = prevScrollVertical === scrollVertical && prevScrollHorizontal === scrollHorizontal;
	            ...
	            var frozenWidth = frozenCellWidth * (this._settings.yProperties.length + dataDirectionToVertical * isShowDataKey);
	        ...
	        Viewer.prototype.renderDataToVertical = function (isForceRender) {
	            ...
	            // #20161230-01 : 값 필드 표시 방향 선택 기능 - End
	            var html = [];
	            var leafColWidth = this._leafColumnWidth; // 20180807 : Koo : Resize Column - S
	            var leafFrozenColWidth = this._leafFrozenColumnWidth;
	            var cellWidth = this._settings.cellWidth;
	            var cellHeight = this._settings.cellHeight;
	            var xPropMax = this._settings.xProperties.length;
	            var yPropMax = this._settings.yProperties.length;
	            ...
	            var frozenWidth = frozenCellWidth * (yPropMax + isShowDataKey);
	            var frozenHeight = cellHeight * frozenHeightCnt;
	            ...
	            if (0 < widthKeys.length) {
	                ...
	                var currentGridWidth = this._elementBody.style.width.replace(/px/gi, '') * 1 - frozenWidth;
	            ...
	            // range.left = Math.floor(this._scrollLeft / cellWidth);
	            {
	                ...
	                var _loop = function _loop(idx, nMax) {
	                    var xItem = _this2._xItems[idx];
	                    var xPropLeafColName = _this2._settings.xProperties.reduce(function (acc, prop) {
	                        var xVal = xItem[prop.name];
	            ...
	            var rowAttributes = void 0;
	            var rowStyles = void 0;
	            var columnAttributes = void 0;
	            var columnStyles = void 0;
	            ...
	            // x축 - y축이 중첩되는 고정 영역 - Start
	            {
	                for (var xpi = 0; xpi < frozenHeightCnt; xpi++) {
	                    ...
	                    var frozenColumnStyleLeft = 0;
	                    for (var ypi = 0, ypiMax = this._settings.yProperties.length + isShowDataKey; ypi < ypiMax; ypi++) {
	                        columnAttributes = {};
	                        columnAttributes["class"] = pivotStyle.cssClass.headCell;
	                        columnStyles = {};

	                        var leafFrozenColWidthKey = "";
	                        ...
	                        var frozenColWidth = leafFrozenColWidth[leafFrozenColWidthKey];
	            ...
	            // Head Wrap : x축 타이틀 표시 - Start
	            {
	                ...
	                var totalWidth = 0;
	                this._xItems.forEach(function (item) {
	                    var leafColumnWidthName = _this2._settings.xProperties.reduce(function (acc, currProp) {
	                        acc = '' === acc ? acc : acc + "||";
	            ...
	            {
	                var _loop2 = function _loop2(_xpi) {
	                    rowAttributes = {};
	                    rowAttributes["class"] = pivotStyle.cssClass.headRow;
	                    rowAttributes["class"] = _this2.addClassFontStyle(rowAttributes["class"], _this2._settings.header.font);
	                    rowAttributes["class"] = _this2.addClassTextAlign(rowAttributes["class"], _this2._settings.header.align, 'LEFT');
	                    rowStyles = {};
	                    rowStyles["width"] = "100%";
	                    rowStyles["height"] = cellHeight + "px";
	                    rowStyles["top"] = (_xpi + (_this2._isPivot ? 1 : 0)) * cellHeight + "px;"; // 원본일때에는 COLUMNS를 표현 x
	                    html.push("<div " + common.attributesString(rowAttributes, rowStyles) + ">");

	                    var prevValue = '';

	                    var _loop3 = function _loop3(xii) {
	                        ...
	                        var arrKeys = [];
	                        var arrVals = [];
	                        for (var idx = 0; idx < _xpi; idx++) {
	                            arrKeys.push(_this2._settings.xProperties[idx].name);
	                            arrVals.push(xItem[_this2._settings.xProperties[idx].name]);
	                        } // end for - xProperties
	                        if (0 < arrKeys.length) {
	                        ...
	                        columnStyles = {};
	                        columnStyles["height"] = cellHeight + "px";
	                        columnStyles["color"] = _this2._settings.header.font.color;
	                        columnStyles["background-color"] = _this2._settings.header.backgroundColor;
	                        // 20180807 : Koo : Resize Column - S
	                        // columnStyles["left"] = (xii * cellWidth) + "px";
	                        // columnStyles["width"] = (colspan * cellWidth) + "px";
	                        var leftPos = 0;
	                        var currLeafColName = columnAttributes["data-parent-vals"] ? columnAttributes["data-parent-vals"] + "||" + xItem[propertyName] : xItem[propertyName];
	                        var xPrevItemList = _this2._xItems.slice(0, xii);
	                        xPrevItemList.map(function (xPrevItem) {
	                            return _this2._settings.xProperties.slice(0, _xpi + 1).map(function (xProp) {
	                        ...
	                        columnStyles["width"] = Object.keys(leafColWidth).reduce(function (acc, currVal) {
	                ...
	                for (var _xpi = 0; _xpi < xPropMax; _xpi++) {
	                    _loop2(_xpi);
	                } // end for - xpi
	            ...
	            html.length = 0;
	            {
	                var predicate = function predicate(a, b, ypi) {
	                    for (var i = ypi; i >= 0; i--) {
	                        var propertyName = _this._settings.yProperties[i].name;
	                ...
	                for (var yii = range.top; yii <= range.bottom; yii++) {
	                    for (var zpi = 0; zpi < zPropMax; zpi++) {
	                        var index = yii * zPropMax + zpi;
	                        // let rowIdx 	= ( zPropMax * ( yii - range.top ) ) + zpi;	// rowIdx 가 무조건 0부터 시작
	                        var rowIdx = zPropMax * yii + zpi; // rowIdx 가 인덱스 번호대로 생성
	                        rowAttributes = {};
	                        rowAttributes["class"] = pivotStyle.cssClass.headRow;
	                        rowAttributes["class"] = this.addClassFontStyle(rowAttributes["class"], this._settings.header.font);
	                        rowAttributes["class"] = this.addClassTextAlign(rowAttributes["class"], this._settings.header.align, 'LEFT');
	                        ...
	                        html.push("<div " + common.attributesString(rowAttributes, rowStyles) + ">");
	                        if (zpi === 0) {
	                            var yItem = this._yItems[yii];
	                            var frozenColumnStylesLeft = 0;
	                            for (var _ypi = 0; _ypi < this._settings.yProperties.length; _ypi++) {
	                                var propertyName = this._settings.yProperties[_ypi].name;
	                                var value = common.format(yItem[propertyName], this._settings.yProperties[_ypi].digits);
	                                var rowspan = 1;
	                                // Add by burgerboy2
	                                // #20160220-01 : 스크롤시 디자인 깨지는 현상 수정
	                                if (index > range.top && this._yItems[yii - 1] && predicate(this._yItems[yii - 1], yItem, _ypi) && html.indexOf(value) > -1) {
	                                	  frozenColumnStylesLeft += leafFrozenColWidth[propertyName];
	                                    continue;
	                                }
	                                for (var i = yii + 1; i < this._yItems.length; i++) {
	                                    if (predicate(this._yItems[i], yItem, _ypi)) {
	                                        rowspan++;
	                                    } else {
	                                        break;
	                                    }
	                                ...
	                                var _arrKeys = [];
	                                var _arrVals = [];
	                                for (var idx = 0; idx < _ypi; idx++) {
	                                    _arrKeys.push(this._settings.yProperties[idx].name);
	                                    _arrVals.push(yItem[this._settings.yProperties[idx].name]);
	                                } // end for - yProperties
	                                if (0 < _arrKeys.length) {
	                                    columnAttributes["data-parent-keys"] = _arrKeys.join("||");
	                                    columnAttributes["data-parent-vals"] = _arrVals.join("||");
	                                }
	                                // #20161229-01 : 축 선택 시 상위 축 정보 포함 제공 - End
	                                ...
	                                var tempCellHeight = rowspan * this._settings.zProperties.length * cellHeight;
	                    ...
	                    if (this._settings.totalValueStyle && yii === this._yItems.length - 1) {
	                        for (var _zpi = 0; _zpi < zPropMax; _zpi++) {
	                            rowAttributes = {};
	                            rowAttributes["class"] = pivotStyle.cssClass.headRow;
	                            ...
	                            if (0 === _zpi && 0 < this._settings.yProperties.length) {
	                                ...
	                                var _tempCellHeight = cellHeight * zPropMax;
	            ...
	            html.length = 0;
	            {
	                ...
	                var stepRangeColors = this._settings.body.color && this._settings.body.color.stepRangeColors ? this._settings.body.color.stepRangeColors : null;
	                for (var _yii = range.top; _yii <= range.bottom; _yii++) {
	                    var _yItem = this._yItems[_yii];
	                    for (var _zpi2 = 0; _zpi2 < zPropMax; _zpi2++) {
	                        var _index = _yii * zPropMax + _zpi2;
	                        // let rowIdx 	= ( zPropMax * ( yii - range.top ) ) + zpi;	// rowIdx 가 무조건 0부터 시작
	                        var _rowIdx = zPropMax * _yii + _zpi2; // rowIdx 가 인덱스 번호대로 생성
	                        var zpiProp = this._settings.zProperties[_zpi2];
	                        var objCriteria = null;
	                        if (showColorStep) {
	                            objCriteria = this._dataCriteria[zpiProp.name];
	                        }
	                        rowAttributes = {};
	                        rowAttributes["class"] = pivotStyle.cssClass.bodyRow + (_index % 2 === 0 ? " odd" : "");
	                        rowAttributes["class"] = this.addClassFontStyle(rowAttributes["class"], this._settings.body.font);
	                        rowAttributes["class"] = this.addClassTextAlign(rowAttributes["class"], this._settings.body.align, 'RIGHT');
	                        rowAttributes["data-rowIdx"] = _rowIdx;
	                        rowStyles = {};
	                        rowStyles["width"] = "100%";
	                        rowStyles["height"] = cellHeight + "px";
	                        rowStyles["top"] = _index * cellHeight + "px;";
	                        html.push("<div " + common.attributesString(rowAttributes, rowStyles) + ">");

	                        var _loop4 = function _loop4(xii) {

	                            var xItem = _this2._xItems[xii];
	                            var context = _this2._itemsContext;
	                            var contains = false;

	                            columnAttributes = {};
	                            columnAttributes["class"] = pivotStyle.cssClass.bodyCell;
	                            columnAttributes["data-rowIdx"] = _rowIdx;
	                            // columnAttributes[ "data-colIdx" ] = ( xii - range.left );		// colIdx 가 무조건 0부터 시작
	                            columnAttributes["data-colIdx"] = xii; // colIdx 가 인덱스 번호대로 시작
	                            ...
	                            var _loop5 = function _loop5(_idx4) {
	                                var tempItem = _this2._xItems[_idx4];
	                                var leafColName = _this2._settings.xProperties.map(function (item) {
	                                    return tempItem[item.name];
	                                }).join('||');
	                                leftPos = leftPos + leafColWidth[leafColName];
	                            };

	                            for (var _idx4 = 0; _idx4 < xii; _idx4++) {
	                                _loop5(_idx4);
	                            }
	                            columnStyles["left"] = leftPos + "px";

	                            var leafColName = '';
	                            _this2._settings.xProperties.forEach(function (prop) {
	                                var xVal = xItem[prop.name];
	                                if (xVal) {
	                            ...
	                            var arrParentKey = []; // 20180807 : Koo : Resize Column
	                            var arrParentVal = []; // 20180807 : Koo : Resize Column
	                            for (var _i = 0; _i < _this2._settings.xProperties.length; _i++) {
	                                var xPropKey = _this2._settings.xProperties[_i].name;
	                            ...
	                            for (var _i2 = 0; _i2 < _this2._settings.yProperties.length; _i2++) {
	                                context = context[_yItem[_this2._settings.yProperties[_i2].name]];
	                            }

	                            try {
	                                if (context) {
	                                    var itemData = context.item[zpiProp.name];
	                                    ...
	                                    for (var key in context.item) {
	                                        // 20171130 taeho - 피봇 / 원본 데이터형태 모두 지원하도록 변경
	                                        columnAttributes["data-item-" + key] = _this2._isPivot ? key + '―' + context.item[key] : context.item[key];
	                                    }
	                                    // #20161227-02 Cell Click Event 추가 : Cell Data 설정 - End

	                                    // Cell Index 추출 및 설정 - Start
	                                    var nItemIdx = -1;
	                                    for (var _idx5 = 0, nMax = items.length; _idx5 < nMax; _idx5++) {
	                                        var objItem = items[_idx5];
	                                        var isUnmatch = false;
	                                        for (var _key4 in context.item) {
	                                            if (objItem[_key4] !== context.item[_key4]) {
	                                                isUnmatch = true;
	                                                break;
	                                            }
	                                    ...
	                                    if ('number' === typeof itemData) {

	                                        // hAlign의 auto인경우 숫자데이터는 오른쪽으로 정렬
	                                        if (_this2._settings.body && _this2._settings.body.align && _this2._settings.body.align.hAlign && 'auto' === _this2._settings.body.align.hAlign.toLowerCase()) {
	                                        ...
	                                        // 단계별 색상 설정 추가 - Start
	                                        if (showColorStep) {
	                                            var strColor = objCriteria.getColor(itemData);
	                                            var strTxtColor = objCriteria.getTextColor(itemData);
	                                            ...
	                                            if (stepRangeColors && stepRangeColors.length > 0) {
	                                                ...
	                                                var _strColor = void 0;
	                                                var _strTxtColor = void 0;

	                                                // 색상타입이 글자일때
	                                                if ('TEXT' === _this2._settings.body.color.colorTarget) {
	                                                    _strColor = '#ffffff';
	                                                    _strTxtColor = objCriteria.getUserRangeColor(itemData, stepRangeColors);

	                                                    // 배경일때
	                                                } else {
	                                                    _strColor = objCriteria.getUserRangeColor(itemData, stepRangeColors);
	                                                    _strTxtColor = '#ffffff';
	                                                }

	                                                // textColor가 있는경우 해당 textColor로 설정
	                                                if (_this2._settings.body.color.stepTextColors && _this2._settings.body.color.stepTextColors.length > 0) {
	                                                    _strTxtColor = _this2._settings.body.color.stepTextColors;
	                                                }

	                                                _strTxtColor && (columnStyles["color"] = _strTxtColor);
	                                                _strColor && (columnStyles["background-color"] = _strColor);
	                                            }
	                                        }
	                                        // 단계별 색상 설정 추가 - End
	                                    }
	                    ...
	                    if (this._settings.totalValueStyle && _yii === this._yItems.length - 1) {

	                        for (var _zpi3 = 0; _zpi3 < zPropMax; _zpi3++) {

	                            var _zpiProp = this._settings.zProperties[_zpi3];
	                            ...
	                            rowStyles["width"] = "100%";
	                            rowStyles["height"] = cellHeight + "px";
	                            rowStyles["top"] = this._yItems.length * zPropMax * cellHeight + _zpi3 * cellHeight + "px;";
	                            html.push("<div " + common.attributesString(rowAttributes, rowStyles) + ">");

	                            var _loop6 = function _loop6(xii) {
	                                var xItem = _this2._xItems[xii];
	                                ...
	                                columnAttributes["class"] = pivotStyle.cssClass.bodyCell;
	                                columnStyles = {};
	                                columnStyles["color"] = _this2._settings.totalValueStyle.font.color;
	                                columnStyles["background-color"] = _this2._settings.totalValueStyle.backgroundColor;
	                                columnStyles["height"] = cellHeight + "px";
	                                ...
	                                var _loop7 = function _loop7(_idx6) {
	                                    var tempItem = _this2._xItems[_idx6];
	                                    var leafColName = _this2._settings.xProperties.map(function (item) {
	                                        return tempItem[item.name];
	                                    }).join('||');
	                                    leftPos = leftPos + leafColWidth[leafColName];
	                                ...
	                                for (var _idx6 = 0; _idx6 < xii; _idx6++) {
	                                    _loop7(_idx6);
	                                }
	                                columnStyles["left"] = leftPos + "px";
	        ...
	        Viewer.prototype.renderDataToHorizontal = function (isForceRender) {
	            ...
	            var yPropMax = this._settings.yProperties.length;
	            var zPropMax = this._settings.zProperties.length;
	            var cellWidthZ = cellWidth * zPropMax;
	            var frozenCellWidth = this._settings.leftAxisWidth ? this._settings.leftAxisWidth : this._settings.cellWidth;
	            var xPropTitleCnt = 0 < xPropMax ? 1 : 0; // #20181116-01 : 상단 공백 타이틀 제거
	            var frozenHeightCnt = xPropMax + isShowDataKey; // #20181116-01 : 상단 공백 타이틀 제거
	            ...
	            if (0 < widthKeys.length) {
	                var contentSizeWidth = widthKeys.reduce(function (acc, item) {
	                    return acc + _this3._leafColumnWidth[item];
	                }, 0);
	                var currentGridWidth = this._elementBody.style.width.replace(/px/gi, '') * 1 - frozenWidth;
	                // if (this.IS_FILL_WIDTH && contentSizeWidth < currentGridWidth) {
	                //     // let cellDiffWidth = (currentGridWidth - contentSizeWidth) / widthKeys.length;
	            ...
	            // range.left = Math.floor(this._scrollLeft / cellWidthZ);
	            {
	                ...
	                var _loop8 = function _loop8(idx, nMax) {
	                    var xItem = _this3._xItems[idx];
	                    var xPropLeafColName = _this3._settings.xProperties.reduce(function (acc, prop) {
	                        var xVal = xItem[prop.name];
	                        xVal && (acc = acc + ('' === acc ? xVal : '||' + xVal));
	                        return acc;
	                    }, '');
	            ...
	            // Head Wrap : x축 타이틀 표시 - Start
	            {
	                var totalWidth = 0;
	                this._xItems.forEach(function (item) {
	                    var leafColumnWidthName = _this3._settings.xProperties.reduce(function (acc, currProp) {
	                        acc = '' === acc ? acc : acc + "||";
	                        return acc + item[currProp.name];
	                    }, '');
	                    _this3._settings.zProperties.forEach(function (zProp) {
	                        var name = '' !== leafColumnWidthName ? leafColumnWidthName + "||" + zProp.name : zProp.name;
	            ...
	            {
	                var _loop9 = function _loop9(_xpi2) {
	                    rowAttributes = {};
	                    rowAttributes["class"] = pivotStyle.cssClass.headRow;
	                    rowAttributes["class"] = _this3.addClassFontStyle(rowAttributes["class"], _this3._settings.header.font);
	                    rowAttributes["class"] = _this3.addClassTextAlign(rowAttributes["class"], _this3._settings.header.align, 'LEFT');
	                    rowStyles = {};
	                    rowStyles["width"] = "100%";
	                    rowStyles["height"] = cellHeight + "px";
	                    rowStyles["top"] = (_xpi2 + xPropTitleCnt) * cellHeight + "px;";
	                    html.push("<div " + common.attributesString(rowAttributes, rowStyles) + ">");

	                    var prevValue = '';

	                    var _loop12 = function _loop12(xii) {
	                        ...
	                        var arrKeys = [];
	                        var arrVals = [];
	                        for (var idx = 0; idx < _xpi2; idx++) {
	                            arrKeys.push(_this3._settings.xProperties[idx].name);
	                            arrVals.push(xItem[_this3._settings.xProperties[idx].name]);
	                        } // end for - xProperties
	                        if (0 < arrKeys.length) {
	                            columnAttributes["data-parent-keys"] = arrKeys.join("||");
	                            columnAttributes["data-parent-vals"] = arrVals.join("||");
	                        ...
	                        columnStyles = {};
	                        columnStyles["height"] = cellHeight + "px";
	                        columnStyles["color"] = _this3._settings.header.font.color;
	                        columnStyles["background-color"] = _this3._settings.header.backgroundColor;
	                        // 20180807 : Koo : Resize Column - S
	                        // columnStyles["left"] = (zPropMax * xii * cellWidth) + "px";
	                        // columnStyles["width"] = (colspan * zPropMax * cellWidth) + "px";
	                        var leftPos = 0;
	                        var currLeafColName = columnAttributes["data-parent-vals"] ? columnAttributes["data-parent-vals"] + "||" + xItem[propertyName] : xItem[propertyName];
	                        var xPrevItemList = _this3._xItems.slice(0, xii);
	                        xPrevItemList.map(function (xPrevItem) {
	                            return _this3._settings.xProperties.slice(0, _xpi2 + 1).map(function (xProp) {
	                                return xPrevItem[xProp.name];
	                            }).join('||');
	                        }).reduce(function (acc, currVal) {
	                            -1 === acc.indexOf(currVal) && acc.push(currVal);
	                ...
	                for (var _xpi2 = 0; _xpi2 < xPropMax; _xpi2++) {
	                    _loop9(_xpi2);
	                } // end for - xpi
	                ...
	                if (this._settings.body.showAxisZ) {
	                    ...
	                    var _loop10 = function _loop10(xii) {

	                        // #20161229-01 : 축 선택 시 상위 축 정보 포함 제공 - Start
	                        var xItem = _this3._xItems[xii];
	                        var strKeys = _this3._settings.xProperties.map(function (xProp) {
	                            return xProp.name;
	                        }).join('||');
	                        var strVals = _this3._settings.xProperties.map(function (xProp) {
	                            return xItem[xProp.name];
	                        }).join('||');
	                        // #20161229-01 : 축 선택 시 상위 축 정보 포함 제공 - End

	                        var _loop11 = function _loop11(zpi) {
	                            // z-axis 추가
	                            columnAttributes = {};
	                            columnAttributes["class"] = pivotStyle.cssClass.headCell;
	                            columnAttributes["title"] = _this3._settings.zProperties[zpi].name;
	                            columnAttributes["data-key"] = 'dataAxis';
	                            ...
	                            columnStyles = {};
	                            columnStyles["height"] = cellHeight + "px";
	                            var zPropName = _this3._settings.zProperties[zpi].name;
	                            ...
	                            var xPrevItemList = _this3._xItems.slice(0, xii);
	                            xPrevItemList.map(function (xPrevItem) {
	                                return _this3._settings.xProperties.map(function (xProp) {
	                                    return xPrevItem[xProp.name];
	                                }).join('||');
	                            }).reduce(function (acc, currVal) {
	                                -1 === acc.indexOf(currVal) && acc.push(currVal);
	                            ...
	                            leftPos = leftPos + _this3._settings.zProperties.slice(0, zpi).reduce(function (acc, currVal) {
	                                var leafColName = columnAttributes["data-parent-vals"] ? columnAttributes["data-parent-vals"] + "||" + currVal.name : currVal.name;
	                                return acc + leafColWidth[leafColName];
	                            }, 0);
	                            columnStyles["left"] = leftPos + "px";

	                            var leafColName = columnAttributes["data-parent-vals"] ? columnAttributes["data-parent-vals"] + "||" + zPropName : zPropName;
	                            columnStyles["width"] = Object.keys(leafColWidth).reduce(function (acc, currVal) {
	                                if (currVal && -1 < currVal.indexOf(leafColName)) {
	                                    acc = acc + leafColWidth[currVal];
	                                }
	            ...
	            html.length = 0;
	            {
	                ...
	                for (var yii = range.top; yii <= range.bottom; yii++) {
	                    ...
	                    for (var _ypi2 = 0; _ypi2 < yPropMax; _ypi2++) {
	                        var propertyName = this._settings.yProperties[_ypi2].name;
	                        var value = common.format(yItem[propertyName], this._settings.yProperties[_ypi2].digits);
	                        var rowspan = 1;
	                        if (index > range.top && this._yItems[yii - 1] && predicate(this._yItems[yii - 1], yItem, _ypi2)) {
	                        	  frozenColumnStylesLeft += leafFrozenColWidth[propertyName];
	                            continue;
	                        }
	                        for (var i = yii + 1; i < this._yItems.length; i++) {
	                            if (predicate(this._yItems[i], yItem, _ypi2)) {
	                                rowspan++;
	                            } else {
	                                break;
	                            }
	                        ...
	                        var _arrKeys2 = [];
	                        var _arrVals2 = [];
	                        for (var idx = 0; idx < _ypi2; idx++) {
	                            _arrKeys2.push(this._settings.yProperties[idx].name);
	                            _arrVals2.push(yItem[this._settings.yProperties[idx].name]);
	                        } // end for - yProperties

	                        if (0 < _arrKeys2.length) {
	                            columnAttributes["data-parent-keys"] = _arrKeys2.join("||");
	                            columnAttributes["data-parent-vals"] = _arrVals2.join("||");
	                        }
	                        // #20161229-01 : 축 선택 시 상위 축 정보 포함 제공 - End
	                        ...
	                        columnStyles["left"] = frozenColumnStylesLeft + "px";
	                        columnStyles["width"] = leafFrozenColWidth[propertyName] + "px";
	                        var tempCellHeight = rowspan * cellHeight;
	                        columnStyles["height"] = tempCellHeight + "px";
	                        // columnStyles["line-height"] = this.getLineHeightForValign( this._settings.header.align, tempCellHeight, rowspan ) + "px !important";
	                        columnStyles["z-index"] = range.bottom - yii; // 영역 클릭 이슈 해결 위해 추가
	                        columnStyles["color"] = this._settings.header.font.color;
	            ...
	            html.length = 0;
	            {
	                ...
	                var items = this._items;
	                var stepRangeColors = this._settings.body.color && this._settings.body.color.stepRangeColors ? this._settings.body.color.stepRangeColors : null;
	                for (var _yii2 = range.top; _yii2 <= range.bottom; _yii2++) {
	                    var _index2 = _yii2;
	                    var _rowIdx2 = _yii2; // rowIdx 가 인덱스 번호대로 생성
	                    rowAttributes = {};
	                    rowAttributes["class"] = pivotStyle.cssClass.bodyRow + (_index2 % 2 === 0 ? " odd" : "");
	                    rowAttributes["class"] = this.addClassFontStyle(rowAttributes["class"], this._settings.body.font);
	                    rowAttributes["class"] = this.addClassTextAlign(rowAttributes["class"], this._settings.body.align, 'RIGHT');
	                    rowAttributes["data-rowIdx"] = _rowIdx2;
	                    rowStyles = {};
	                    rowStyles["width"] = "100%";
	                    rowStyles["height"] = cellHeight + "px";
	                    rowStyles["top"] = _index2 * cellHeight + "px;";
	                    html.push("<div " + common.attributesString(rowAttributes, rowStyles) + ">");

	                    var _yItem2 = this._yItems[_yii2];
	                    for (var xii = range.left; xii <= range.right; xii++) {
	                        var _loop13 = function _loop13(zpi) {
	                            var zpiProp = _this3._settings.zProperties[zpi];
	                            var objCriteria = null;
	                            if (showColorStep) {
	                                objCriteria = _this3._dataCriteria[zpiProp.name];
	                            }

	                            var xItem = _this3._xItems[xii];
	                            var context = _this3._itemsContext;
	                            var contains = false;

	                            columnAttributes = {};
	                            columnAttributes["class"] = pivotStyle.cssClass.bodyCell;
	                            columnAttributes["data-rowIdx"] = _rowIdx2;
	                            ...
	                            leftPos = leftPos + _this3._settings.zProperties.slice(0, zpi).reduce(function (acc, currVal) {
	                                var leafColName = '' !== xPropLeafColName ? xPropLeafColName + "||" + currVal.name : currVal.name;
	                                return acc + leafColWidth[leafColName];
	                            }, 0);
	                            columnStyles["left"] = leftPos + "px";
	                            if ('' === xPropLeafColName) {
	                            ...
	                            try {
	                                ...
	                                var _arrParentKey = []; // 20180807 : Koo : Resize Column
	                                var _arrParentVal = []; // 20180807 : Koo : Resize Column
	                                for (var _i3 = 0; _i3 < xPropMax; _i3++) {
	                                    var xPropKey = _this3._settings.xProperties[_i3].name;
	                                    context = context[xItem[xPropKey]];
	                                    _arrParentKey.push(xPropKey); // 20180807 : Koo : Resize Column
	                                    _arrParentVal.push(xItem[xPropKey]); // 20180807 : Koo : Resize Column
	                                }
	                                columnAttributes["data-parent-keys"] = _arrParentKey.join('||'); // 20180807 : Koo : Resize Column
	                                columnAttributes["data-parent-vals"] = _arrParentVal.join('||'); // 20180807 : Koo : Resize Column
	                                for (var _i4 = 0; _i4 < yPropMax; _i4++) {
	                                    context = context[_yItem2[_this3._settings.yProperties[_i4].name]];
	                                }
	                                ...
	                                if (context) {
	                                    ...
	                                    for (var key in context.item) {
	                                        ...
	                                        // 20171130 taeho - 피봇 / 원본 데이터형태 모두 지원하도록 변경
	                                        columnAttributes["data-item-" + key] = _this3._isPivot ? key + '―' + context.item[key] : context.item[key];
	                                    }
	                                    // #20161227-02 Cell Click Event 추가 : Cell Data 설정 - End

	                                    // Cell Index 추출 및 설정 - Start
	                                    var nItemIdx = -1;
	                                    for (var _idx7 = 0, nMax = items.length; _idx7 < nMax; _idx7++) {
	                                        var objItem = items[_idx7];
	                                        var isUnmatch = false;
	                                        for (var _key5 in context.item) {
	                                            if (objItem[_key5] !== context.item[_key5]) {
	                                                isUnmatch = true;
	                                                break;
	                                            }
	                                        } // end for - context.item
	                                    ...
	                                    if ('number' === typeof itemData) {
	                                        ...
	                                        if (showColorStep) {
	                                            ...
	                                            if (stepRangeColors && stepRangeColors.length > 0) {
	                                                ...
	                                                var _strColor2 = void 0;
	                                                var _strTxtColor2 = void 0;

	                                                // 색상타입이 글자일때
	                                                if ('TEXT' === _this3._settings.body.color.colorTarget) {
	                                                    _strColor2 = '#ffffff';
	                                                    _strTxtColor2 = objCriteria.getUserRangeColor(itemData, stepRangeColors);

	                                                    // 배경일때
	                                                } else {
	                                                    _strColor2 = objCriteria.getUserRangeColor(itemData, stepRangeColors);
	                                                    _strTxtColor2 = '#ffffff';
	                                                }

	                                                // textColor가 있는경우 해당 textColor로 설정
	                                                if (_this3._settings.body.color.stepTextColors && _this3._settings.body.color.stepTextColors.length > 0) {
	                                                    _strTxtColor2 = _this3._settings.body.color.stepTextColors;
	                                                }

	                                                _strTxtColor2 && (columnStyles["color"] = _strTxtColor2);
	                                                _strColor2 && (columnStyles["background-color"] = _strColor2);
	                                            }
	                                        }
	                                        // 단계별 색상 설정 추가 - End
	                                    }
	                    ...
	                    // 요약 정보 데이터 표시 설정 - Start
	                    if (this._settings.totalValueStyle && _yii2 === this._yItems.length - 1) {
	                        rowAttributes = {};
	                        rowAttributes["class"] = pivotStyle.cssClass.bodyRow;
	                        rowAttributes["class"] = this.addClassFontStyle(rowAttributes["class"], this._settings.totalValueStyle.font);
	                        rowAttributes["class"] = this.addClassTextAlign(rowAttributes["class"], this._settings.totalValueStyle.align, 'RIGHT');
	                        ...
	                        for (var _xii = range.left; _xii <= range.right; _xii++) {
	                            var _loop14 = function _loop14(zpi) {
	                                var zpiProp = _this3._settings.zProperties[zpi];
	                                var xItem = _this3._xItems[_xii];

	                                // cell설정
	                                columnAttributes = {};
	                                columnAttributes["class"] = pivotStyle.cssClass.bodyCell;
	                                columnStyles = {};
	                                columnStyles["color"] = _this3._settings.totalValueStyle.font.color;
	                                columnStyles["background-color"] = _this3._settings.totalValueStyle.backgroundColor;
	                                columnStyles["height"] = cellHeight + "px";
	                                ...
	                                var leftPos = 0;
	                                var xPrevItemList = _this3._xItems.slice(0, _xii);
	                                xPrevItemList.map(function (xPrevItem) {
	                                    return _this3._settings.xProperties.map(function (xProp) {
	                                        return xPrevItem[xProp.name];
	                                    }).join('||');
	                                ...
	                                var summaryKey = '' === xPropLeafColName ? zpiProp.name : xPropLeafColName + '||' + zpiProp.name;
	         ...
	         */
	        Viewer.prototype.onScroll = function (e) {

	            var scrollTop = this._elementBody.scrollTop;
	            var scrollTopChanged = this._scrollTop !== scrollTop;
	            var scrollLeft = this._elementBody.scrollLeft;
	        ...
	        Viewer.prototype.resize = function () {
	            ...
	            var redraw = function redraw() {
	                var widthKeys = Object.keys(_this4._leafColumnWidth);
	                var contentSizeWidth = widthKeys.reduce(function (acc, item) {
	                ...
	                if (contentSizeWidth <= currentGridWidth && 0 < widthKeys.length) {
	                    widthKeys.forEach(function (key) {
	                        _this4._leafColumnWidth[key] = currentGridWidth * (_this4._leafColumnWidth[key] / contentSizeWidth);
	                    });
	                }
	                _this4.arrange();
	            };
	        ...
	        Viewer.prototype.getSummaryValue = function (dataList) {
	            ...
	            if (summarySettings && summarySettings.aggregationType) {
	                ...
	                switch (summarySettings.aggregationType) {
	                    case "SUM":
	                        summaryValue = dataList.reduce(function (acc, val) {
	                            return acc + val;
	                        }, 0);
	                        break;
	                    case "AVERAGE":
	        ...
	        Viewer.prototype.arrangeFrozenColumnRelatedElements = function () {
	        	var _this2 = this;
	        	var frozenWidthWidthKeys = Object.keys(this._leafFrozenColumnWidth);
            	var frozenWidth = frozenWidthWidthKeys.reduce(function (acc, item) {
