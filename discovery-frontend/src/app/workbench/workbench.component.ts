...
export class WorkbenchComponent extends AbstractComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public supportIMSI: boolean = false;
  ...
  public ngOnInit() {
    ...
    this.subscriptions.push(
      ...
      this.broadCaster.on<any>('SHOW_HIVE_PERSONAL_DATABASE_CREATION_DATA_AGGREGATE_TASK').subscribe((dataAggregate: DataAggregate) => {
        this.creationDataAggregateTaskComponent.init(this.workbench.dataConnection.id, this.detailWorkbenchDatabase.getAllDatabases(),
      ...
      if (data.valid) {
        ...
        this.workspaceService.getWorkSpace(data.workspace.id, 'forDetailView').then((workspace: Workspace) => {
          ...
          if (workspace.active && permissionChecker.isViewWorkbench()) {
            ...
            if (data.dataConnection.supportIMSI) {
              this.supportIMSI = true;
            }
